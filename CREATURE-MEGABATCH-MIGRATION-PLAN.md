...
| 5a | ✅ METADATA + ENGINE HOOK | 60 | baf9ece | Lair Actions (137 creatures). Found + downloaded legendarygroups.json from 5etools-mirror-3 repo. Parser extracts lair action options + initiative count. Engine fires lair action log at start of each round (initiative count 20). v1: logs only — does NOT apply mechanical effects (each lair action is bespoke — HIGH-risk, deferred). 12 test assertions. |
