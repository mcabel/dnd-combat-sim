...
//
// Effect: Choose one object weighing 1 to 5 pounds within range that isn't being worn or carried. The object flies in a straight line up to 90 feet in a direction you choose before falling to the ground, stoppi
//
...
/**
 * Returns true if the caster should cast Catapult this turn.
 *
 * Preconditions:
 *   - Caster has 'Catapult' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Catapult-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Catapult')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Catapult')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Catapult');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Catapult! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Catapult. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
