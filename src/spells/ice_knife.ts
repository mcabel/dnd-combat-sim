...
//
// Effect: You create a shard of ice and fling it at one creature within range. Make a ranged spell attack against the target. On a hit, the target takes  piercing damage. Hit or miss, the shard th
//
...
/**
 * Returns true if the caster should cast Ice Knife this turn.
 *
 * Preconditions:
 *   - Caster has 'Ice Knife' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Ice Knife-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Ice Knife')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Ice Knife')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Ice Knife');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Ice Knife! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Ice Knife. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
