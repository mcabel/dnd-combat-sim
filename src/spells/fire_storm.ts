...
export function execute(caster: Combatant, targets: Combatant[], state: EngineState): void {
  ...
  emit(
    ...
    `${caster.name} casts Fire Storm! (DC ${saveDC} DEX, ${metadata.dieCount}d${metadata.dieSides} ${metadata.damageType}, ${metadata.aoeRadiusFt}-ft radius AoE) — ${effectiveTargets.length} creature${effectiveTargets.length !== 1 ? 's' : ''} caught${excludedCount > 0 ? ` (${excludedCount} excluded by Globe of Invulnerability)` : ''}!`,
  );
  ...
  for (const target of effectiveTargets) {
    ...
    // Sorcerer 6) adds CHA mod to the fire damage if the caster's
    // ancestry is fire. Bonus is added once before save halving.
