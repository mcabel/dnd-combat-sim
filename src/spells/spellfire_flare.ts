// ============================================================
// Spellfire Flare — FRHoF p.146
//
// 1-level evocation, 1 action, range 60 ft.
// Duration: Instantaneous.
//
// Effect: You unleash a blast of brilliant fire. Make a ranged spell attack against a target within range; a target gains no benefit from  or {@variantrule Cover|XPHB|Three-Q
//
...
export const metadata = {
  ...
  concentration: false,
  castingTime: 'action',
  spellfireFlareV1Simplified: true,
} as const;
...
/**
 * Returns true if the caster should cast Spellfire Flare this turn.
 *
 * Preconditions:
 *   - Caster has 'Spellfire Flare' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Spellfire Flare-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Spellfire Flare')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Spellfire Flare')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Spellfire Flare');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Spellfire Flare! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Spellfire Flare. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
