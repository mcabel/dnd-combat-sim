...
//         before this spell ends, your weapon pierces both body and
//         mind, and the attack deals an extra 1d6 psychic damage to
//         the target. The target must succeed on a Wisdom saving
//         throw or be frightened of you until the spell ends. As an
//         action, the frightened creature can make a Wisdom check
//         against your spell save DC to steel itself and end the spell.
//
...
//     `condition: 'frightened'` is applied to the target on hit
//     (sourceIsConcentration: true — ends if Wrathful Smite's conc
//     breaks). Documented via `wrathfulSmiteCanonV1Implemented: true`.
//   - Initial WIS save to resist the frightened effect (PHB p.289)
//     NOT modelled — v1 auto-frightens on hit. Documented via
//     `wrathfulSmiteRidersV1Simplified: true` and
//     `wrathfulSmiteEndOfTurnSaveV1Simplified: true` (the action-WIS-
//     check-to-end-fear escape clause is also simplified out).
...
export const metadata = {
  ...
  wrathfulSmiteCanonV1Implemented: true,
  wrathfulSmiteRidersV1Simplified: true,             // initial WIS save simplified
  wrathfulSmiteEndOfTurnSaveV1Simplified: true,      // action-WIS-check escape clause simplified
} as const;
 ...
 *  3. Start concentration on 'Wrathful Smite'.
 *  4. Set `_nextHitRider` on the caster (one-shot rider).
 *
 * The rider is CONSUMED by resolveAttack's damage branch in combat.ts
 * on the next weapon hit. The damage branch rolls 1d6 psychic (crit
 * doubles), adds it to the damage total, applies `frightened` to the
...
export function execute(caster: Combatant, state: EngineState): void {
  ...
  startConcentration(caster, 'Wrathful Smite');
  ...
  caster._nextHitRider = {
    ...
    condition: 'frightened' as Condition,
  };

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Wrathful Smite! (Next weapon hit: +${metadata.count}d${metadata.dieSides} ${metadata.damageType} + frightened)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} readies Wrathful Smite — next weapon hit deals +${metadata.count}d${metadata.dieSides} ${metadata.damageType}!`,
    caster.id,
  );
}
...
/**
 * Cleanup hook for Wrathful Smite — clears a stale `_nextHitRider`
 * (whose spellName is 'Wrathful Smite') if concentration broke before
...
export function cleanup(c: Combatant): void {
  if (
    c._nextHitRider?.spellName === 'Wrathful Smite' &&
    (!c.concentration?.active || c.concentration.spellName !== 'Wrathful Smite')
  ) {
