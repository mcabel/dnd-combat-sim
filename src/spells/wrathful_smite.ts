...
// Duration: 1 minute.
//
// Effect: The next time you hit with a melee weapon attack during this spell's duration, your attack deals an extra  psychic damage. Additionally, if the target is a creature, it must make a Wisdom
//
...
export const metadata = {
  ...
  wrathfulSmiteV1Simplified: true,
} as const;
 ...
 *   - Caster is NOT already Wrathful Smite-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Wrathful Smite')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Wrathful Smite')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Wrathful Smite');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Wrathful Smite! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Wrathful Smite. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
