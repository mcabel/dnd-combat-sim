...
// Duration: 1 minute.
//
// Effect: You touch a creature. The creature's  is tripled until the spell ends.
//
...
/**
 * Returns true if the caster should cast Jump this turn.
 *
 * Preconditions:
 *   - Caster has 'Jump' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Jump-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Jump')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Jump')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Jump');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Jump! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Jump. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
