...
//
// 1-level necromancy, 1 action, range Touch (5 ft).
// Duration: Instantaneous.
//
// Effect: Make a melee spell attack against a creature you can reach. On a hit, the target takes  necrotic damage.
//
...
export const metadata = {
  ...
  concentration: false,
  castingTime: 'action',
  inflictWoundsV1Simplified: true,
} as const;
...
/**
 * Returns true if the caster should cast Inflict Wounds this turn.
 *
 * Preconditions:
 *   - Caster has 'Inflict Wounds' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Inflict Wounds-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Inflict Wounds')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Inflict Wounds')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Inflict Wounds');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Inflict Wounds! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Inflict Wounds. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
