...
//
// 1-level evocation, 1 action, range 90 ft.
// Duration: Instantaneous.
//
// Effect: You hurl a 4-inch-diameter sphere of energy at a creature that you can see within range. You choose acid, cold, fire, lightning, poison, or thunder for the type of orb you create, and then make a rang
//
...
/**
 * Returns true if the caster should cast Chromatic Orb this turn.
 *
 * Preconditions:
 *   - Caster has 'Chromatic Orb' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Chromatic Orb-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Chromatic Orb')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Chromatic Orb')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Chromatic Orb');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Chromatic Orb! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Chromatic Orb. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
