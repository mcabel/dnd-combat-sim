...
//
// Effect: You hurl an undulating, warbling mass of chaotic energy at one creature in range. Make a ranged spell attack against the target. On a hit, the target takes  damage. Choose one of th
//
...
/**
 * Returns true if the caster should cast Chaos Bolt this turn.
 *
 * Preconditions:
 *   - Caster has 'Chaos Bolt' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Chaos Bolt-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Chaos Bolt')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Chaos Bolt')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Chaos Bolt');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Chaos Bolt! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Chaos Bolt. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
