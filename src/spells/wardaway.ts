// ============================================================
// Wardaway — FRHoF p.147
//
// 1-level abjuration, 1 action, range 60 ft.
// Duration: Instantaneous.
//
// Effect: You hurl a disorienting magical force toward one creature within range. The target makes a Constitution saving throw; Constructs and Undead automatically succeed on this save.
//
...
/**
 * Returns true if the caster should cast Wardaway this turn.
 *
 * Preconditions:
 *   - Caster has 'Wardaway' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Wardaway-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Wardaway')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Wardaway')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Wardaway');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Wardaway! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Wardaway. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
