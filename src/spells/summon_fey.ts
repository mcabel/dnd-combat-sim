// ============================================================
// Summon Fey — TCE p.112
//
// 3rd-level conjuration, action, range 30 ft, concentration (1 hr).
// Components: V, S, M (a feather or tuft of fur from a fey
//             creature).
//
// Effect: You call forth a fey spirit. It manifests in an
//         unoccupied space that you can see within range. This
//         corporeal form uses the Fey Spirit stat block. When
//         you cast this spell, choose a mood: "Fey Spirit".
//
...
//
// Fey Spirit Stat Block (TCE p.112):
//   AC: 12 + level of spell
//   HP: 30 + 10 per spell level above 3rd (30 at L3, 40 at L4, etc.)
//   Speed: 40 ft
//   STR 10, DEX 14, CON 12, INT 8, WIS 12, CHA 14
//   Attack: Shortsword +5, 1d6+2 piercing + 1d6 psychic
//   At L5+: Multiattack (2 attacks)
//   Special: Misty Step once per turn as bonus action
//
// v1 simplifications:
//   - Misty Step NOT modelled (just melee attack)
//   - Always picks 'Fey Spirit' option (no mood variants)
...
// manually as Combatant objects via createFeySpirit().
//
...
export const metadata = {
  ...
  concentration: true,
  castingTime: 'action',
  summonFeyV1Implemented: true,
  summonFeyUpcastV1Implemented: true,
} as const;
...
// ---- Fey Spirit stat block builder --------------------------

/**
 * Create a Fey Spirit Combatant.
 * TCE summon stat blocks are NOT in the bestiary — they are built manually.
 *
 * @param caster    - the combatant who cast Summon Fey
 * @param slotLevel - the spell slot level used (3–9)
 */
export function createFeySpirit(
  caster: Combatant,
  slotLevel: number,
): Combatant {
  const hp = 30 + 10 * (slotLevel - 3);
  const ac = 12 + slotLevel;
  const numAttacks = slotLevel >= 5 ? 2 : 1;

  // Build melee attack action(s)
  // Shortsword: +5 to hit, 1d6+2 piercing + 1d6 psychic
  const attackAction: Action = {
    ...
    damageType: 'piercing',
    // Psychic rider modelled as a second damage instance — v1: folded into
    // description only; the engine's damage resolver handles the primary dice.
    saveDC: null,
    saveAbility: null,
    isAoE: false,
    isControl: false,
    ...
    costType: 'action',
    legendaryCost: 0,
    description: `Shortsword: +5 to hit, 1d6+2 piercing plus 1d6 psychic${numAttacks > 1 ? ' (Multiattack: 2 attacks)' : ''}`,
  };

  // Position: adjacent to caster (1 square away)
  const pos = { x: caster.pos.x + 1, y: caster.pos.y, z: caster.pos.z };

  const id = `summon_fey_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
  ...
  for (let i = 0; i < numAttacks; i++) {
    actions.push({
      ...attackAction,
      name: numAttacks > 1 ? `Shortsword (${i + 1}/${numAttacks})` : 'Shortsword',
    });
  }

  return {
    id,
    name: `Fey Spirit (${caster.name})`,
    isPlayer: false,
...
/**
 * Returns true if the caster should cast Summon Fey.
 *
 * Preconditions:
 *   - Caster has 'Summon Fey' in their actions
 *   - Caster has at least a 3rd-level spell slot available
 *   - Caster is NOT already concentrating on any spell
 *   - Caster doesn't already have a Summon Fey active
 */
export function shouldCast(caster: Combatant, bf: Battlefield): boolean {
  if (caster.concentration?.active) return false;
  if (!caster.actions.some(a => a.name === 'Summon Fey')) return false;
  if (!hasSpellSlot(caster, 3)) return false;

  // Check if caster already has a Summon Fey active
  const existingSummon = [...bf.combatants.values()].some(
    c => c.isSummon && c.summonerId === caster.id && c.summonSpellName === 'Summon Fey'
  );
 ...
 *  3. Start concentration on Summon Fey.
 *  4. Create the Fey Spirit combatant (built manually, NOT from bestiary).
...
export function execute(
  ...
  startConcentration(caster, 'Summon Fey');

  const summon = createFeySpirit(caster, slotLevel);
  ...
  emit(
    ...
    `${caster.name} casts Summon Fey (slot L${slotLevel})! Fey Spirit appears with ${summon.maxHP} HP, AC ${summon.ac}.`,
    summon.id,
  );
}
