...
// Heroism — PHB p.250
//
// 1-level enchantment, 1 action, range Touch (5 ft), concentration.
// Duration: 1 minute.
//
// Effect: A willing creature you touch is imbued with bravery. Until the spell ends, the creature is immune to being  and gains temporary hit points equal to your spellcasting ability mod
//
...
/**
 * Returns true if the caster should cast Heroism this turn.
 *
 * Preconditions:
 *   - Caster has 'Heroism' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Heroism-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Heroism')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Heroism')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Heroism');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Heroism! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Heroism. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
