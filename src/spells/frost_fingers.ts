...
export function execute(
  ...
  emit(
    ...
    `${caster.name} casts Frost Fingers! (DC ${saveDC} CON, ${metadata.dieCount}d${metadata.dieSides} ${metadata.damageType}, ${CONE_RANGE_FT}-ft cone) — ${targets.length} creature${targets.length !== 1 ? 's' : ''} caught!`,
  );
