...
//
// 1-level evocation, 1 action, range 10 ft.
// Duration: Instantaneous.
//
// Effect: You cause a tremor in the ground within range. Each creature other than you in that area must make a Dexterity saving throw. On a failed save, a creature takes  bludgeoning damage and is
//
...
/**
 * Returns true if the caster should cast Earth Tremor this turn.
 *
 * Preconditions:
 *   - Caster has 'Earth Tremor' in their actions
 *   - Caster has at least one 1-level-or-higher slot available
 *   - Caster is NOT already Earth Tremor-active (re-cast would be a no-op in v1)
 */
export function shouldCast(caster: Combatant, _bf: Battlefield): boolean {
  if (!caster.actions.some(a => a.name === 'Earth Tremor')) return false;
  if (!hasSpellSlot(caster, 1)) return false;
  if (caster._genericSpellActiveSpells?.has('Earth Tremor')) return false;
  return true;
}
...
export function execute(
  ...
  caster._genericSpellActiveSpells.add('Earth Tremor');

  emit(
    state, 'action', caster.id,
    `${caster.name} casts Earth Tremor! (v1: forward-compat flag set; mechanical effect not yet implemented)`,
    caster.id,
  );
  emit(
    state, 'condition_add', caster.id,
    `${caster.name} is affected by Earth Tremor. (v1: forward-compat flag set; no mechanical effect until engine subsystem is implemented)`,
    caster.id,
  );
}
