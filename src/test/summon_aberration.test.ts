// ============================================================
// summon_aberration.test.ts — Summon Aberration spell module (TCE p.110)
// 4th-level conjuration, action, range 30 ft, concentration 1 hr.
// Effect: Spawns an Aberrant Spirit combatant that shares the caster's
//         initiative count and takes its turn immediately after.
//         HP/AC scale with slot level. Disappears on concentration
//         break or 0 HP. v1: always Slaad option.
//
...
// break despawn, HP/AC scaling, and Slaad stat block.
// ============================================================

import { shouldCast, execute, metadata, createAberrantSpirit } from '../spells/summon_aberration';
...
const SUMMON_ABERRATION_ACTION: Action = {
  ...
  costType: 'action',
  legendaryCost: 0,
  description: 'Summon Aberration (concentration 1 hr)',
};
...
function makeCaster(id: string = 'caster1', pos: Vec3 = { x: 0, y: 0, z: 0 }): Combatant {
  return makeCombatant(id, {
    name: 'Wizard',
    pos,
    actions: [SUMMON_ABERRATION_ACTION],
...
eq('name is Summon Aberration', metadata.name, 'Summon Aberration');
...
eq('summonAberrationV1Implemented is true', metadata.summonAberrationV1Implemented, true);
eq('summonAberrationUpcastV1Implemented is true', metadata.summonAberrationUpcastV1Implemented, true);
...

{
  ...
  assert('Returns false when caster has no Summon Aberration action', shouldCast(caster, bf) === false);
}
...

{
  ...
  assert('Returns false when caster already has a Summon Aberration active', shouldCast(caster, bf) === false);
}
...
// ============================================================
// 3. createAberrantSpirit — combatant creation (Slaad)
// ============================================================
...
{
  const caster = makeCaster();
  const spirit = createAberrantSpirit(caster, 4);

  eq('isSummon is true', spirit.isSummon, true);
  eq('summonerId matches caster', spirit.summonerId, 'caster1');
  eq('summonSpellName is Summon Aberration', spirit.summonSpellName, 'Summon Aberration');
  eq('faction matches caster', spirit.faction, 'party');
  eq('name includes Aberrant Spirit', spirit.name.includes('Aberrant Spirit'), true);
  eq('name includes caster name', spirit.name.includes('Wizard'), true);
  ...
  eq('flySpeed is null (Slaad)', spirit.flySpeed, null);
  eq('STR is 16', spirit.str, 16);
  eq('DEX is 10', spirit.dex, 10);
  eq('CON is 15', spirit.con, 15);
  ...
  eq('attack name is Claw (Slaad)', spirit.actions[0].name, 'Claw');
...

{
  ...
  if (summons.length === 1) {
    ...
    eq('Summon summonSpellName is Summon Aberration', summon.summonSpellName, 'Summon Aberration');
  ...
  eq('Caster concentrating on Summon Aberration', caster.concentration?.spellName, 'Summon Aberration');
...

{
  ...
  if (actionEvents.length >= 1) {
    assert('Action event mentions "Summon Aberration"', actionEvents[0].description.includes('Summon Aberration'));
    assert('Action event mentions "Aberrant Spirit"', actionEvents[0].description.includes('Aberrant Spirit'));
