// ============================================================
// spellfire_storm.test.ts — Spellfire Storm bespoke spell module (Session 24)
// SCAG p.150: 4th-level evocation, action, range 60 ft. Canon: concentration
// up to 1 min + DoT. v1: concentration + DoT simplified to one-shot.
// Effect: AUTO-HIT 4d10 fire damage. NO attack roll, NO saving throw — the
// target always takes the damage. (v1 simplifications: canon +spellcasting
// mod is NOT added [unlike Spellfire Flare]; canon DoT NOT modelled; upcast
// NOT modelled.)
//
// Migrated from the Session 20 generic dispatch registry in Session 24.
// Mirrors spellfire_flare.test.ts structure (auto-hit single-target) but
// with Spellfire Storm's stats (L4, 4d10 fire, NO +mod, 60-ft range). Uses
// withSlots4.
//
...
//   - NO save_success/save_fail events are emitted (no save)
//   - only 'action' + 'damage' log events appear
//   - rollDamage() takes NO mod args (NO +spellcasting mod scaling —
//     unlike Spellfire Flare's rollDamage(includeMod, spellcastingMod))
// ============================================================

import { shouldCast, execute, metadata, rollDamage } from '../spells/spellfire_storm';
...
/**
 * Spellfire Storm action. attackType: 'save' is fine (the spell ignores
...
const SS_ACTION: Action = {
  ...
  costType: 'action',
  legendaryCost: 0,
  description: 'Spellfire Storm (AUTO-HIT 4d10 fire, no save, no attack, no +mod)',
};
...
/** Sorcerer at pos (0,0,0) with Spellfire Storm + 2 4th-level slots */
...
eq('Name is Spellfire Storm', metadata.name, 'Spellfire Storm');
...
// 2a. No Spellfire Storm action → null
{
  const caster = makeCombatant('sorc', { actions: [], resources: withSlots4(2) });
  const enemy = makeEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
  eq('Returns null when caster lacks Spellfire Storm action', shouldCast(caster, bf), null);
}
// 2b. No 4th-level slots → null
{
  const caster = makeCombatant('sorc', { actions: [SS_ACTION], resources: withSlots4(0) });
  const enemy = makeEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
...
// 2c. No enemies in range → null
{
  ...
  const enemy = makeEnemy('e1', { x: 13, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
  eq('Returns null when no enemies in range (65 ft > 60 ft)', shouldCast(caster, bf), null);
}
...

{
  ...
  if (target) {
    ...
    // 4b. Damage applied: 4d10 (NO +mod) range 4-40
    const dmgDealt = hpBefore - enemy.currentHP;
    assert(`Damage in 4d10 range (4-40, NO +mod): got ${dmgDealt}`,
    ...
    // No condition rider for Spellfire Storm
...
// ---- 5. execute — multiple runs always apply damage -------------

console.log('\n=== 5. execute — auto-hit always applies damage ===\n');

{
  ...
  for (let i = 0; i < 50; i++) {
    const caster = makeCaster({ x: 0, y: 0, z: 0 });
    caster.resources = withSlots4(50);   // plenty of slots
...
// ---- 6. rollDamage — NO +mod scaling ----------------------------

console.log('\n=== 6. rollDamage ===\n');

// rollDamage() takes NO args — always returns 4d10 (range 4-40). Unlike
// Spellfire Flare, there is no includeMod/spellcastingMod scaling.
{
...

{
  ...
  let cleanupOk = true;
  try { (require('../spells/spellfire_storm') as any).cleanup(caster); }
