// command.test.ts — Command (Session 25 / Batch 2)
// PHB p.223: L1, 60 ft, WIS save or incapacitated, NO concentration (commands simplified).
import { shouldCast, execute, metadata } from '../spells/command';
...
console.log('\n=== 1. Metadata ===\n');
eq('Name', metadata.name, 'Command'); eq('Level 1', metadata.level, 1); eq('Range 60', metadata.rangeFt, 60); eq('Save wis', metadata.saveAbility, 'wis'); eq('NOT concentration', metadata.concentration, false);
console.log('\n=== 2. shouldCast gates ===\n');
...
{ const c = makeCaster(); const e = weak('e1', { x: 5, y: 0 }); const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf); if (t) { execute(c, t as Combatant, st); eq('slot consumed', (c.resources as any).spellSlots[1].remaining, 0); assert('NOT concentrating', !(c.concentration?.active)); assert('incapacitated applied', e.conditions.has('incapacitated')); } }
console.log('\n=== 5. execute — guaranteed success ===\n');
{ const c = makeCaster({ x: 0, y: 0, z: 0 }, CMD_ACTION_LOW); const e = strong('e1', { x: 5, y: 0 }); const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf); if (t) { execute(c, t as Combatant, st); assert('NOT incapacitated', !e.conditions.has('incapacitated')); const ss = st.log.events.filter((x: any) => x.type === 'save_success'); assert('save_success log', ss.length === 1); } }
console.log('\n=== 6. Cleanup no-op ===\n');
{ let ok = true; try { (require('../spells/command') as any).cleanup(makeCaster()); } catch { ok = false; } assert('no throw', ok); }
