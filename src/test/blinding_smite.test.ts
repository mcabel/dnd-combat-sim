// ============================================================
// blinding_smite.test.ts
//
// Tests:
//   1. Metadata (7 assertions — no riders-simplified flag for blinding_smite)
...
//
// Run: npx ts-node --transpile-only src/test/blinding_smite.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/blinding_smite';
...

{
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Blinding Smite');
}

{
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Blinding Smite');
  eq('3e: rider.dieSides', caster._nextHitRider!.dieSides, 8);
  eq('3f: rider.count', caster._nextHitRider!.count, 3);
  eq('3g: rider.damageType', caster._nextHitRider!.damageType, 'radiant');
  eq('3h: rider.condition', caster._nextHitRider!.condition, 'blinded');
}

{
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Blinding Smite'));
...

{
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Blinding Smite'));
...

{
  ...
  caster.concentration = { active: true, spellName: 'Blinding Smite', dcIfHit: 10 };
  cleanup(caster);
