...
//
// Run: npx ts-node --transpile-only src/test/searing_smite.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/searing_smite';
...
eq('1a: name', metadata.name, 'Searing Smite');
...
eq('1g: canon flag true', metadata.searingSmiteCanonV1Implemented, true);
eq('1h: riders-simplified flag true', metadata.searingSmiteRidersV1Simplified, true);
...

{
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Searing Smite');
}

{
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Searing Smite');
  eq('3e: rider.dieSides', caster._nextHitRider!.dieSides, 6);
  eq('3f: rider.count', caster._nextHitRider!.count, 1);
  eq('3g: rider.damageType', caster._nextHitRider!.damageType, 'fire');
  assert('3h: no condition (DoT simplified)', caster._nextHitRider!.condition === undefined);
}

{
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Searing Smite'));
...

{
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Searing Smite'));
