...
//
// Run: npx ts-node --transpile-only src/test/ensnaring_strike.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/ensnaring_strike';
...
eq('1a: name', metadata.name, 'Ensnaring Strike');
...
eq('1g: canon flag true', metadata.ensnaringStrikeCanonV1Implemented, true);
eq('1h: riders-simplified flag true', metadata.ensnaringStrikeRidersV1Simplified, true);
...
{
  // 3b: concentration starts on 'Ensnaring Strike'
  const caster = makeCaster('ranger');
  const bf = makeBF([caster]);
  const state = makeState(bf);
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Ensnaring Strike');
}

{
  // 3c-g: _nextHitRider set with correct fields
  const caster = makeCaster('ranger');
  const bf = makeBF([caster]);
  const state = makeState(bf);
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Ensnaring Strike');
  ...
  eq('3h: rider.condition', caster._nextHitRider!.condition, 'restrained');
}

{
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Ensnaring Strike'));
...

{
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Ensnaring Strike'));
...

{
  ...
  caster.concentration = { active: true, spellName: 'Ensnaring Strike', dcIfHit: 10 };
  cleanup(caster);
