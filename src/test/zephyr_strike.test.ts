...
//
// Run: npx ts-node --transpile-only src/test/zephyr_strike.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/zephyr_strike';
...
eq('1a: name', metadata.name, 'Zephyr Strike');
...
eq('1g: canon flag true', metadata.zephyrStrikeCanonV1Implemented, true);
eq('1h: riders-simplified flag true', metadata.zephyrStrikeRidersV1Simplified, true);
...

{
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Zephyr Strike');
}

{
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Zephyr Strike');
  ...
  assert('3h: no condition (disengage/speed/adv riders simplified)',
...

{
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Zephyr Strike'));
...

{
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Zephyr Strike'));
...

{
  ...
  caster._nextHitRider = {
    ...
    dieSides: 8, count: 1, damageType: 'force',
  };
...

{
  ...
  caster.concentration = { active: true, spellName: 'Zephyr Strike', dcIfHit: 10 };
  cleanup(caster);
