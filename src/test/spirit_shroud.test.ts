...
//
// Run: npx ts-node --transpile-only src/test/spirit_shroud.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/spirit_shroud';
...
eq('1a: name', metadata.name, 'Spirit Shroud');
eq('1b: level', metadata.level, 3);
eq('1c: concentration true', metadata.concentration, true);
eq('1d: dieSides 8', metadata.dieSides, 8);
eq('1e: count 1', metadata.count, 1);
eq('1f: damageType radiant (v1 picks radiant over necrotic)', metadata.damageType, 'radiant');
eq('1g: canon flag true', metadata.spiritShroudCanonV1Implemented, true);
eq('1h: riders-simplified flag true', metadata.spiritShroudRidersV1Simplified, true);
...

{
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Spirit Shroud');
}

{
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Spirit Shroud');
  eq('3e: rider.dieSides', caster._nextHitRider!.dieSides, 8);
  eq('3f: rider.count', caster._nextHitRider!.count, 1);
  eq('3g: rider.damageType', caster._nextHitRider!.damageType, 'radiant');
  assert('3h: no condition (aura/slow riders simplified)',
...

{
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Spirit Shroud'));
...

{
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Spirit Shroud'));
...

{
  ...
  caster._nextHitRider = {
    ...
    dieSides: 8, count: 1, damageType: 'radiant',
  };
...

{
  ...
  caster.concentration = { active: true, spellName: 'Spirit Shroud', dcIfHit: 10 };
  cleanup(caster);
