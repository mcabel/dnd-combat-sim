// ============================================================
// erupting_earth.test.ts — Erupting Earth bespoke spell module (Session 24)
// XGE p.155: 3rd-level transmutation, action, range 60 ft, NO concentration.
// Effect: AoE DEX save. On fail: 3d12 bludgeoning. On success: half.
// AoE: 20-ft cube (v1: approximated as 20-ft radius centred on the
// highest-threat enemy within 60 ft — mirrors shatter.ts pattern with
// a 20-ft radius instead of 10 ft). Difficult-terrain rider NOT modelled.
//
// Migrated from the Session 20 generic dispatch registry in Session 24.
// Mirrors shatter.test.ts structure (AoE radius save) but with Erupting
// Earth's stats (L3, DEX save, 3d12 bludgeoning, 60-ft range, 20-ft
// radius). Uses withSlots3.
//
...
//   - (4,0,0) = 20 ft from caster (within 20-ft radius around center)
//   - (5,0,0) = 25 ft from caster (OUT of 20-ft radius around (0,0,0))
// ============================================================

import { shouldCast, execute, metadata, rollDamage } from '../spells/erupting_earth';
...
const EE_ACTION: Action = {
  ...
  costType: 'action',
  legendaryCost: 0,
  description: 'Erupting Earth (DEX save, 3d12 bludgeoning, 60-ft range, 20-ft radius AoE)',
};

const EE_ACTION_LOW_DC: Action = {
  ...EE_ACTION,
...
/** Druid at pos (0,0,0) with Erupting Earth + 2 3rd-level slots */
function makeCaster(pos: Vec3 = { x: 0, y: 0, z: 0 }, action: Action = EE_ACTION): Combatant {
...
eq('Name is Erupting Earth', metadata.name, 'Erupting Earth');
...
// 2a. No Erupting Earth action → null
{
  const caster = makeCombatant('druid', { actions: [], resources: withSlots3(2) });
  const enemy = makeWeakEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
  eq('Returns null when caster lacks Erupting Earth action', shouldCast(caster, bf), null);
}
// 2b. No 3rd-level slots → null
{
  const caster = makeCombatant('druid', { actions: [EE_ACTION], resources: withSlots3(0) });
  const enemy = makeWeakEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
...
// ---- 4. execute — guaranteed fail (full 3d12 damage) -----------
...

{
  ...
  if (targets) {
    ...
    assert(`Damage in 3d12 range (3-36): got ${dmgDealt}`,
    ...
    // No condition rider for Erupting Earth
...
{
  const caster = makeCaster({ x: 0, y: 0, z: 0 }, EE_ACTION_LOW_DC);
  ...
  if (targets) {
    ...
    // Half of 3d12 (floor), range 1-18
    const dmgDealt = hpBefore - enemy.currentHP;
    assert(`Half-damage in 1-18 range: got ${dmgDealt}`,
      dmgDealt >= 1 && dmgDealt <= 18);
...

{
  ...
  if (targets) {
    ...
    assert('e1 took damage in 3d12 range (3-36)', e1Lost >= 3 && e1Lost <= 36, `got ${e1Lost}`);
    assert('e2 took damage in 3d12 range (3-36)', e2Lost >= 3 && e2Lost <= 36, `got ${e2Lost}`);
    assert('e3 took damage in 3d12 range (3-36)', e3Lost >= 3 && e3Lost <= 36, `got ${e3Lost}`);
    const saveFails = state.log.events.filter((e: any) => e.type === 'save_fail');
...

{
  ...
  let cleanupOk = true;
  try { (require('../spells/erupting_earth') as any).cleanup(caster); }
  catch { cleanupOk = false; }
  assert('cleanup() does not throw', cleanupOk);
}

// ---- 8. rollDamage respects 3d12 -------------------------------
