...

{
  ...
  // L8 upcast: Session 41 Task #3 bestiary integration — originally picked
  // Unicorn (CR 5 celestial, MM p.294) when only MM+DMG were loaded.
  // Session 53: with the expanded bestiary (99 sourcebooks), the L8 pick is
  // the alphabetically-first CR 5 celestial — Battleforce Angel (GGR).
  // Falls back to Couatl (CR 4) if bestiary is not available.
  // We assert CR + summon-shape but don't pin the name (varies by loaded sources).
  ...
  if (summon8) {
    // Bestiary is loaded from ./bestiaryData → some CR 5 celestial.
    ...
    if (isCouatl) {
      ...
      // Bestiary path: a CR 5 celestial (Unicorn, Battleforce Angel, etc.)
      eq('L8 upcast: summon CR is 5 (bestiary celestial)', summon8.cr, 5);
      assert('L8 upcast: summon maxHP ≥ 1', summon8.maxHP >= 1);
    }
  }
