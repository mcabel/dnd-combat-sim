// ============================================================
// hail_of_thorns.test.ts
//
// Tests:
//   1. Metadata (8 assertions)
//   2. shouldCast — planner precondition gates (5 assertions)
//   3. execute — slot/concentration/rider pipeline (8 assertions)
//   4. cleanup — stale-rider clearing + no-op safety (3 assertions)
//
// Run: npx ts-node --transpile-only src/test/hail_of_thorns.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/hail_of_thorns';
...
eq('1a: name', metadata.name, 'Hail of Thorns');
eq('1b: level', metadata.level, 1);
eq('1c: concentration true', metadata.concentration, true);
eq('1d: dieSides 10', metadata.dieSides, 10);
eq('1e: count 1', metadata.count, 1);
eq('1f: damageType piercing', metadata.damageType, 'piercing');
eq('1g: canon flag true', metadata.hailOfThornsCanonV1Implemented, true);
eq('1h: riders-simplified flag true', metadata.hailOfThornsRidersV1Simplified, true);
...
{
  // 2a: returns false when caster has no actions
  const caster = makeCaster('ranger');
  caster.actions = [];
...
{
  // 2b: returns false when caster has no slots
  const caster = makeCaster('ranger');
...
{
  // 2c: returns false when already concentrating
  const caster = makeCaster('ranger');
...
{
  // 2d: returns false when already has a pending _nextHitRider
  const caster = makeCaster('ranger');
  caster._nextHitRider = {
    spellName: 'Ensnaring Strike',
    dieSides: 6, count: 1, damageType: 'piercing', condition: 'restrained',
  };
  const bf = makeBF([caster]);
...
{
  // 3a: consumes a 1st-level spell slot
  const caster = makeCaster('ranger');
  const bf = makeBF([caster]);
...
{
  // 3b: concentration starts on 'Hail of Thorns'
  const caster = makeCaster('ranger');
  const bf = makeBF([caster]);
  const state = makeState(bf);
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Hail of Thorns');
}

{
  // 3c-h: _nextHitRider set with correct fields (no condition)
  const caster = makeCaster('ranger');
  const bf = makeBF([caster]);
  const state = makeState(bf);
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Hail of Thorns');
  eq('3e: rider.dieSides', caster._nextHitRider!.dieSides, 10);
  eq('3f: rider.count', caster._nextHitRider!.count, 1);
  eq('3g: rider.damageType', caster._nextHitRider!.damageType, 'piercing');
  assert('3h: no condition (AoE splash simplified)', caster._nextHitRider!.condition === undefined);
}

{
  // 3i: action event logged
  const caster = makeCaster('ranger');
  const bf = makeBF([caster]);
  const state = makeState(bf);
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Hail of Thorns'));
  assert('3i: action event logged', actionEvent !== undefined);
}

{
  // 3j: condition_add event logged
  const caster = makeCaster('ranger');
  const bf = makeBF([caster]);
  const state = makeState(bf);
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Hail of Thorns'));
...
{
  // 4a: clears stale rider when concentration broke
  const caster = makeCaster('ranger');
  caster._nextHitRider = {
    spellName: 'Hail of Thorns',
    dieSides: 10, count: 1, damageType: 'piercing',
  };
  caster.concentration = null;   // concentration broke
  cleanup(caster);
...
{
  // 4b: does NOT clear rider when concentration is still active on this spell
  const caster = makeCaster('ranger');
  caster._nextHitRider = {
    spellName: 'Hail of Thorns',
    dieSides: 10, count: 1, damageType: 'piercing',
  };
  caster.concentration = { active: true, spellName: 'Hail of Thorns', dcIfHit: 10 };
  cleanup(caster);
...
{
  // 4c: cleanup no-op doesn't throw when no rider present
  const caster = makeCaster('ranger');
  let threw = false;
