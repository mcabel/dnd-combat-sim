// flesh_to_stone.test.ts — Flesh to Stone (Session 25 / Batch 2)
// PHB p.241: L6, 60 ft, CON save or restrained, concentration (3-fail petrified simplified).
...
console.log('\n=== 4. execute — guaranteed fail (restrained) ===\n');
{
  ...
  if (t) { execute(c, t as Combatant, st); eq('slot consumed', (c.resources as any).spellSlots[6].remaining, 0); assert('concentration started', c.concentration?.active === true); assert('restrained applied', e.conditions.has('restrained')); }
}
...
console.log('\n=== 5. execute — guaranteed success ===\n');
{
  ...
  const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf);
  if (t) { execute(c, t as Combatant, st); assert('NOT restrained', !e.conditions.has('restrained')); const ss = st.log.events.filter(x => x.type === 'save_success'); assert('save_success log', ss.length === 1); }
}
