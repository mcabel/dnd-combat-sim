// phantasmal_killer.test.ts — Phantasmal Killer (Session 25 / Batch 2)
// PHB p.265: L4, 120 ft, WIS save or frightened + 4d10 psychic, concentration (DoT one-shot).
import { shouldCast, execute, metadata, rollDamage } from '../spells/phantasmal_killer';
...
const PK_ACTION: Action = { name: 'Phantasmal Killer', isMultiattack: false, attackType: 'save', reach: 5, range: { normal: 120, long: 120 }, hitBonus: null, damage: null, damageType: null, saveDC: 25, saveAbility: 'wis', isAoE: false, isControl: true, requiresConcentration: true, slotLevel: 4, costType: 'action', legendaryCost: 0, description: 'Phantasmal Killer' };
const PK_ACTION_LOW: Action = { ...PK_ACTION, saveDC: 5 };
...
function makeCaster(pos: any = { x: 0, y: 0, z: 0 }, a: Action = PK_ACTION) { return makeCombatant('wiz', { name: 'Caster', pos, actions: [a], resources: withSlots4(1) }); }
...
console.log('\n=== 1. Metadata ===\n');
eq('Name', metadata.name, 'Phantasmal Killer'); eq('Level 4', metadata.level, 4); eq('Range 120', metadata.rangeFt, 120);
eq('Die 4d10', metadata.dieCount, 4); eq('Damage psychic', metadata.damageType, 'psychic');
eq('Save wis', metadata.saveAbility, 'wis'); eq('Concentration', metadata.concentration, true);
console.log('\n=== 2. shouldCast gates ===\n');
{ const c = makeCombatant('wiz', { actions: [], resources: withSlots4(1) }); eq('null: no action', shouldCast(c, makeBF([c, weak('e1', { x: 1, y: 0 })])), null); }
{ const c = makeCombatant('wiz', { actions: [PK_ACTION], resources: withSlots4(0) }); eq('null: no slots', shouldCast(c, makeBF([c, weak('e1', { x: 1, y: 0 })])), null); }
...
console.log('\n=== 4. execute — guaranteed fail (4d10 + frightened) ===\n');
{ const c = makeCaster(); const e = weak('e1', { x: 5, y: 0 }, { maxHP: 1000, currentHP: 1000 }); const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf); if (t) { const hp = e.currentHP; execute(c, t as Combatant, st); eq('slot consumed', (c.resources as any).spellSlots[4].remaining, 0); assert('concentration started', c.concentration?.active === true); const d = hp - e.currentHP; assert(`4d10 damage (4-40): ${d}`, d >= 4 && d <= 40); assert('frightened applied', e.conditions.has('frightened')); } }
console.log('\n=== 5. execute — guaranteed success (no damage, no frighten) ===\n');
{ const c = makeCaster({ x: 0, y: 0, z: 0 }, PK_ACTION_LOW); const e = strong('e1', { x: 5, y: 0 }, { maxHP: 1000, currentHP: 1000 }); const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf); if (t) { const hp = e.currentHP; execute(c, t as Combatant, st); eq('NO damage (save negates)', e.currentHP, hp); assert('NOT frightened', !e.conditions.has('frightened')); } }
console.log('\n=== 6. Cleanup no-op ===\n');
{ let ok = true; try { (require('../spells/phantasmal_killer') as any).cleanup(makeCaster()); } catch { ok = false; } assert('no throw', ok); }
console.log('\n=== 7. rollDamage ===\n');
{ let mn = Infinity, mx = -Infinity; for (let i = 0; i < 1000; i++) { const r = rollDamage(); if (r < mn) mn = r; if (r > mx) mx = r; } assert(`min ≥ 4 (${mn})`, mn >= 4); assert(`max ≤ 40 (${mx})`, mx <= 40); }
