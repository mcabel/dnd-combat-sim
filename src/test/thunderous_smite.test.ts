...
//
// Run: npx ts-node --transpile-only src/test/thunderous_smite.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/thunderous_smite';
...
eq('1a: name', metadata.name, 'Thunderous Smite');
...

{
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Thunderous Smite');
}

{
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Thunderous Smite');
...

{
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Thunderous Smite'));
...

{
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Thunderous Smite'));
