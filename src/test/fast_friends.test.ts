// fast_friends.test.ts — Fast Friends (Session 25 / Batch 2)
// EGtW p.151: L3, 30 ft, WIS save or suggestion effect (charmed + disadv on attacks), concentration.
import { shouldCast, execute, metadata } from '../spells/fast_friends';
...
const FF_ACTION: Action = { name: 'Fast Friends', isMultiattack: false, attackType: 'save', reach: 5, range: { normal: 30, long: 30 }, hitBonus: null, damage: null, damageType: null, saveDC: 25, saveAbility: 'wis', isAoE: false, isControl: true, requiresConcentration: true, slotLevel: 3, costType: 'action', legendaryCost: 0, description: 'Fast Friends' };
const FF_ACTION_LOW: Action = { ...FF_ACTION, saveDC: 5 };
...
function makeCaster(pos: any = { x: 0, y: 0, z: 0 }, a: Action = FF_ACTION) { return makeCombatant('wiz', { name: 'Caster', pos, actions: [a], resources: withSlots3(1) }); }
...
console.log('\n=== 1. Metadata ===\n');
eq('Name', metadata.name, 'Fast Friends'); eq('Level 3', metadata.level, 3); eq('Range 30', metadata.rangeFt, 30); eq('Save wis', metadata.saveAbility, 'wis'); eq('Concentration', metadata.concentration, true);
console.log('\n=== 2. shouldCast gates ===\n');
{ const c = makeCombatant('wiz', { actions: [], resources: withSlots3(1) }); eq('null: no action', shouldCast(c, makeBF([c, weak('e1', { x: 1, y: 0 })])), null); }
{ const c = makeCombatant('wiz', { actions: [FF_ACTION], resources: withSlots3(0) }); eq('null: no slots', shouldCast(c, makeBF([c, weak('e1', { x: 1, y: 0 })])), null); }
...
console.log('\n=== 4. execute — guaranteed fail (suggestion effect) ===\n');
{ const c = makeCaster(); const e = weak('e1', { x: 5, y: 0 }); const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf); if (t) { execute(c, t as Combatant, st); eq('slot consumed', (c.resources as any).spellSlots[3].remaining, 0); assert('concentration started', c.concentration?.active === true); assert('suggestion effect applied', e.activeEffects.some(ef => ef.effectType === 'suggestion' && ef.spellName === 'Fast Friends')); assert('charmed condition applied', e.conditions.has('charmed')); } }
console.log('\n=== 5. execute — guaranteed success ===\n');
{ const c = makeCaster({ x: 0, y: 0, z: 0 }, FF_ACTION_LOW); const e = strong('e1', { x: 5, y: 0 }); const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf); if (t) { execute(c, t as Combatant, st); assert('NOT suggestion-affected', !e.activeEffects.some(ef => ef.effectType === 'suggestion')); assert('NOT charmed', !e.conditions.has('charmed')); const ss = st.log.events.filter((x: any) => x.type === 'save_success'); assert('save_success log', ss.length === 1); } }
console.log('\n=== 6. Cleanup no-op ===\n');
{ let ok = true; try { (require('../spells/fast_friends') as any).cleanup(makeCaster()); } catch { ok = false; } assert('no throw', ok); }
