// ============================================================
// lightning_bolt.test.ts — Lightning Bolt bespoke spell module
// (Session 21)
// PHB p.255: 3rd-level evocation, action, range 100 ft (self → 100-ft
// line), NO concentration. Effect: DEX save. On fail: 8d6 lightning.
// On success: half. AoE: 100-ft × 5-ft line from caster toward the
// highest-threat enemy within 100 ft (uses the new inLineFt helper).
//
// Migrated from the Session 19 generic dispatch registry in Session 21.
// Mirrors fireball.test.ts but with Lightning Bolt's stats (L3, 8d6
// lightning, 100-ft × 5-ft line, DEX save).
//
...
import { shouldCast, execute, metadata, rollDamage } from '../spells/lightning_bolt';
...
const LB_ACTION: Action = {
  ...
  costType: 'action',
  legendaryCost: 0,
  description: 'Lightning Bolt (DEX save, 8d6 lightning, 100-ft × 5-ft line)',
};

const LB_ACTION_LOW_DC: Action = {
  ...LB_ACTION,
...
/** Wizard at pos (0,0,0) with Lightning Bolt + 2 3rd-level slots */
function makeWizard(pos: Vec3 = { x: 0, y: 0, z: 0 }, action: Action = LB_ACTION): Combatant {
...
eq('Name is Lightning Bolt', metadata.name, 'Lightning Bolt');
...
// 2a. No Lightning Bolt action → null
{
  const caster = makeCombatant('wiz', { actions: [], resources: withSlots3(2) });
  const enemy = makeWeakEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
  eq('Returns null when caster lacks Lightning Bolt action', shouldCast(caster, bf), null);
}
// 2b. No 3rd-level slots → null
{
  const caster = makeCombatant('wiz', { actions: [LB_ACTION], resources: withSlots3(0) });
  const enemy = makeWeakEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
...
// perpendicular distance <= 2.5 ft → only same-row cells qualify).
{
  ...
  if (result) {
    ...
    eq('Total 4 targets caught (4 on-axis + 0 off-axis)', result.length, 4);
  }
}
...
// 3b. Threat selection — highest maxHP is the aim point
{
  ...
  if (result) {
    eq('Both on-axis enemies caught', result.length, 2);
  }
}
...
{
  const caster = makeWizard({ x: 0, y: 0, z: 0 }, LB_ACTION_LOW_DC);
...

{
  ...
  const targets = shouldCast(caster, bf);
  assert('shouldCast returns 3 targets', targets !== null && targets.length === 3);
  if (targets) {
    execute(caster, targets, state);
    const saveFails = state.log.events.filter(e => e.type === 'save_fail');
...

{
  ...
  let cleanupOk = true;
  try { (require('../spells/lightning_bolt') as any).cleanup(caster); }
