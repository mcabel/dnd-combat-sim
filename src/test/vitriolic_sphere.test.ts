// ============================================================
// vitriolic_sphere.test.ts — Vitriolic Sphere bespoke spell module (Session 24)
// XGE p.168 / EGtW p.165: 4th-level evocation, action, range 150 ft,
// NO concentration.
// Effect: AoE DEX save. On fail: 10d4 acid. On success: half. 20-ft-radius
// sphere centered on the highest-threat enemy within 150 ft. (v1
// simplifications: canon 5d4 end-of-next-turn DoT NOT modelled; upcast NOT
// modelled.)
//
...
// radius save) but with Vitriolic Sphere's stats (L4, DEX save, 10d4 acid,
...
import { shouldCast, execute, metadata, rollDamage } from '../spells/vitriolic_sphere';
...
const VS_ACTION: Action = {
  ...
  costType: 'action',
  legendaryCost: 0,
  description: 'Vitriolic Sphere (DEX save, 10d4 acid, 150-ft range, 20-ft radius AoE)',
};

const VS_ACTION_LOW_DC: Action = {
  ...VS_ACTION,
...
/** Wizard at pos (0,0,0) with Vitriolic Sphere + 2 4th-level slots */
function makeCaster(pos: Vec3 = { x: 0, y: 0, z: 0 }, action: Action = VS_ACTION): Combatant {
...
eq('Name is Vitriolic Sphere', metadata.name, 'Vitriolic Sphere');
...
// 2a. No Vitriolic Sphere action → null
{
  const caster = makeCombatant('wiz', { actions: [], resources: withSlots4(2) });
  const enemy = makeWeakEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
  eq('Returns null when caster lacks Vitriolic Sphere action', shouldCast(caster, bf), null);
}
// 2b. No 4th-level slots → null
{
  const caster = makeCombatant('wiz', { actions: [VS_ACTION], resources: withSlots4(0) });
  const enemy = makeWeakEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
...
// ---- 4. execute — guaranteed fail (full 10d4 damage) -----------
...

{
  ...
  if (targets) {
    ...
    assert(`Damage in 10d4 range (10-40): got ${dmgDealt}`,
      dmgDealt >= 10 && dmgDealt <= 40);
    ...
    // No condition rider for Vitriolic Sphere
...
{
  const caster = makeCaster({ x: 0, y: 0, z: 0 }, VS_ACTION_LOW_DC);
  ...
  if (targets) {
    ...
    // Half of 10d4 (floor), range 5-20
    const dmgDealt = hpBefore - enemy.currentHP;
    assert(`Half-damage in 5-20 range: got ${dmgDealt}`,
      dmgDealt >= 5 && dmgDealt <= 20);
...

{
  ...
  if (targets) {
    ...
    assert('e1 took damage in 10d4 range (10-40)', e1Lost >= 10 && e1Lost <= 40, `got ${e1Lost}`);
    assert('e2 took damage in 10d4 range (10-40)', e2Lost >= 10 && e2Lost <= 40, `got ${e2Lost}`);
    assert('e3 took damage in 10d4 range (10-40)', e3Lost >= 10 && e3Lost <= 40, `got ${e3Lost}`);
    const saveFails = state.log.events.filter((e: any) => e.type === 'save_fail');
...

{
  ...
  let cleanupOk = true;
  try { (require('../spells/vitriolic_sphere') as any).cleanup(caster); }
  catch { cleanupOk = false; }
  assert('cleanup() does not throw', cleanupOk);
}

// ---- 8. rollDamage respects 10d4 -------------------------------
