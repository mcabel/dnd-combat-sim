// ============================================================
// Test: Word of Radiance Cantrip
// XGE p.171 — Level 0 evocation cantrip (caster-centered 5-ft AoE, CON save, radiant)
//
// Tests:
//   1. metadata correctness
//   2. metadata exposes scaling info (5/11/17 → 2d6/3d6/4d6)
//   3. metadata exposes saveAbility = 'con' for AI/parser (CANON — XGE p.171)
//   4. metadata exposes components (V + M holy symbol — no S)
...
//   8. resolveCantripAoE registered — 'Word of Radiance' routes to execute handler
...
//  16. execute integration — Word of Radiance Action on caster.actions drives saveDC
//  17. damage type is RADIANT (rare among cantrips — only Sacred Flame also deals radiant)
//  18. v1 simplification: allyExclusionV1Implemented = false (affects all in range)
//
// Run: npx ts-node src/test/word_of_radiance.test.ts
// ============================================================

import { metadata, execute, WORD_OF_RADIANCE_RADIUS_FT } from '../spells/word_of_radiance';
import {
...
// A Word of Radiance Action as the AI/parser would build it from metadata.
// Save-based: attackType='save', saveDC = caster's spell save DC, saveAbility='con'.
// Damage 1d6 radiant. Caster-centered 5-ft AoE (isAoE = true, range = 5 ft).
const WORD_OF_RADIANCE_ACTION: Action = {
  name: 'Word of Radiance',
  isMultiattack: false,
  attackType: 'save',
  reach: 0,
  range: { normal: WORD_OF_RADIANCE_RADIUS_FT, long: WORD_OF_RADIANCE_RADIUS_FT },
  hitBonus: null,
  ...
  costType: 'action',
  legendaryCost: 0,
  description: 'Word of Radiance',
};

// Save-SUCCESS variant: DC=1 + con=30 (+10) → guaranteed save SUCCESS.
const WORD_OF_RADIANCE_SUCCESS_ACTION: Action = { ...WORD_OF_RADIANCE_ACTION, saveDC: 1 };
...
console.log('\n--- 1. metadata ---');
{
  eq('1a. name', metadata.name, 'Word of Radiance');
  ...
  eq('1f. damageType = radiant', metadata.damageType, 'radiant');
...
// ============================================================
// 3. save ability exposed for AI/parser — CANON (XGE p.171: CON save)
// ============================================================
console.log('\n--- 3. save ability (canon: CON, per XGE p.171) ---');
{
  eq('3a. saveAbility = con', metadata.saveAbility, 'con');
}

// ============================================================
// 4. components: V + M (a holy symbol — no S) — XGE p.171
// ============================================================
console.log('\n--- 4. components ---');
{
  eq('4a. verbal component', metadata.components.v, true);
  eq('4b. no somatic component', metadata.components.s, false);
  eq('4c. material component (a holy symbol)', metadata.components.m, true);
}
...
console.log('\n--- 6. no CANTRIP_EFFECTS entry ---');
{
  ...
  dispatchCantrip(caster, target, 'Word of Radiance', state);
...
console.log('\n--- 7. not a self-buff ---');
{
  ...
  eq('7a. resolveCantripAction returns false', resolveCantripAction(caster, 'Word of Radiance', state), false);
...
console.log('\n--- 8. resolveCantripAoE integration ---');
{
  const caster = makeCombatant('cleric', {
    pos: { x: 5, y: 5, z: 0 },
    actions: [WORD_OF_RADIANCE_ACTION],
  });
  ...
  const ret = resolveCantripAoE(caster, 'Word of Radiance', state);
  ...
  assert('8c. save_fail mentions CON', saveFail?.description.includes('CON') === true, true);
}
...
console.log('\n--- 11. multiple enemies in range ---');
{
  ...
  // enemy3 (1 square diagonal = ~7.07 ft) is OUT of range (Euclidean > 5).
  // So only enemy1 and enemy2 should roll saves.
  ...
  // Both enemies took radiant damage (save FAIL → full 1d6 = 1..6).
  const dmgEvents = state.log.events.filter(
    (e: CombatEvent) => e.type === 'damage' && e.description.includes('radiant'),
  );
  eq('11b. exactly 2 damage events (radiant)', dmgEvents.length, 2);
  for (const d of dmgEvents) {
    assert('11c. radiant damage in 1..6 range',
...
console.log('\n--- 14. 0 creatures in range ---');
{
  ...
  const castLog = state.log.events.find(
    (e: CombatEvent) => e.type === 'action' && e.description.includes('Word of Radiance'),
  );
...
console.log('\n--- 15. save SUCCESS → half, FAIL → full ---');
{
  ...
  const caster = makeCombatant('cleric', {
    pos: { x: 5, y: 5, z: 0 },
    actions: [WORD_OF_RADIANCE_ACTION], // DC=30, FAIL
  });
  const enemy = makeCombatant('goblin', {
    pos: { x: 6, y: 5, z: 0 },
    con: 10, currentHP: 100, maxHP: 100, faction: 'enemy',
...
{
  // DC=1 + con=30 (+10) → guaranteed SUCCESS → half of 1d6 = 0..3 (floored).
  const caster = makeCombatant('cleric', {
    pos: { x: 5, y: 5, z: 0 },
    actions: [WORD_OF_RADIANCE_SUCCESS_ACTION], // DC=1, SUCCESS
  });
  const enemy = makeCombatant('goblin', {
    pos: { x: 6, y: 5, z: 0 },
    con: 30, // +10 mod → save 11+ vs DC 1 → always succeeds
...
console.log('\n--- 16. saveDC read from caster.actions ---');
{
  ...
  const customAction: Action = { ...WORD_OF_RADIANCE_ACTION, saveDC: 20 };
  const caster = makeCombatant('cleric', {
    pos: { x: 5, y: 5, z: 0 },
  ...
  const enemy = makeCombatant('goblin', {
    pos: { x: 6, y: 5, z: 0 },
    con: 10, // +0 mod → save 1..20 vs DC 20
    currentHP: 100, maxHP: 100, faction: 'enemy',
  });
  const bf = makeBF([caster, enemy]);
  const state = makeState(bf);
...
}
{
  ...
  const caster = makeCombatant('cleric', {
    pos: { x: 5, y: 5, z: 0 },
    actions: [], // no Word of Radiance Action
  });
  const enemy = makeCombatant('goblin', {
    pos: { x: 6, y: 5, z: 0 },
    con: 10, currentHP: 100, maxHP: 100, faction: 'enemy',
...
// ============================================================
// 17. damage type is RADIANT (rare among cantrips — only Sacred Flame also)
// ============================================================
console.log('\n--- 17. damage type = radiant ---');
{
  const caster = makeCombatant('cleric', {
    pos: { x: 5, y: 5, z: 0 },
    actions: [WORD_OF_RADIANCE_ACTION],
  ...
  // All damage events should mention "radiant".
  const dmgEvents = state.log.events.filter((e: CombatEvent) => e.type === 'damage');
  for (const d of dmgEvents) {
    assert('17a. damage event mentions radiant', d.description.includes('radiant'),
      `got: ${d.description}`);
  }
  // The Action itself should have damageType = 'radiant'.
  eq('17b. Action damageType = radiant', WORD_OF_RADIANCE_ACTION.damageType, 'radiant');
}

// ============================================================
// 18. v1 simplification: allyExclusionV1Implemented = false
// ============================================================
console.log('\n--- 18. v1 ally-exclusion simplification ---');
{
  eq('18a. allyExclusionV1Implemented = false (v1 affects all in range)',
    metadata.allyExclusionV1Implemented, false);

  // Behavioral test: an ALLY within 5 ft of the caster is also hit
  // (v1 does NOT support the "each creature of your choice" clause).
  const caster = makeCombatant('cleric', {
    pos: { x: 5, y: 5, z: 0 },
    actions: [WORD_OF_RADIANCE_ACTION],
  });
  const ally = makeCombatant('fighter', {
    ...
    faction: 'party', // same faction as caster — v1 still hits
  });
  ...
  // v1: the ally is hit (no ally exclusion).
  assert('18b. v1: ally in range IS hit (no ally exclusion)',
    ally.currentHP < 100, `ally HP=${ally.currentHP}`);
}
