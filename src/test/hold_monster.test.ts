// hold_monster.test.ts — Hold Monster (Session 25 / Batch 2)
// PHB p.251: L5, 60 ft, WIS save or paralyzed, concentration, any creature.
import { shouldCast, execute, metadata } from '../spells/hold_monster';
...
const HM_ACTION: Action = { name: 'Hold Monster', isMultiattack: false, attackType: 'save', reach: 5, range: { normal: 60, long: 60 }, hitBonus: null, damage: null, damageType: null, saveDC: 25, saveAbility: 'wis', isAoE: false, isControl: true, requiresConcentration: true, slotLevel: 5, costType: 'action', legendaryCost: 0, description: 'Hold Monster' };
const HM_ACTION_LOW: Action = { ...HM_ACTION, saveDC: 5 };
...
function makeCaster(pos: any = { x: 0, y: 0, z: 0 }, a: Action = HM_ACTION) { return makeCombatant('wiz', { name: 'Caster', pos, actions: [a], resources: withSlots5(1) }); }
...
console.log('\n=== 1. Metadata ===\n');
eq('Name', metadata.name, 'Hold Monster'); eq('Level 5', metadata.level, 5);
...
{ const c = makeCombatant('wiz', { actions: [HM_ACTION], resources: withSlots5(0) }); eq('null: no slots', shouldCast(c, makeBF([c, weak('e1', { x: 1, y: 0 })])), null); }
...
{ const c = makeCaster(); const par = weak('par', { x: 1, y: 0 }, { maxHP: 300 }); par.conditions.add('paralyzed' as Condition); const fr = weak('fr', { x: 5, y: 0 }, { maxHP: 50 }); const r = shouldCast(c, makeBF([c, par, fr])); if (r) eq('skips paralyzed', (r as Combatant).id, 'fr'); }

console.log('\n=== 4. execute — guaranteed fail (paralyzed) ===\n');
{ const c = makeCaster(); const e = weak('e1', { x: 5, y: 0 }); const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf); if (t) { execute(c, t as Combatant, st); eq('slot consumed', (c.resources as any).spellSlots[5].remaining, 0); assert('concentration started', c.concentration?.active === true); assert('paralyzed applied', e.conditions.has('paralyzed')); } }

console.log('\n=== 5. execute — guaranteed success ===\n');
{ const c = makeCaster({ x: 0, y: 0, z: 0 }, HM_ACTION_LOW); const e = strong('e1', { x: 5, y: 0 }); const bf = makeBF([c, e]); const st = makeState(bf); const t = shouldCast(c, bf); if (t) { execute(c, t as Combatant, st); assert('NOT paralyzed', !e.conditions.has('paralyzed')); const ss = st.log.events.filter((x: any) => x.type === 'save_success'); assert('save_success log', ss.length === 1); } }

console.log('\n=== 6. Cleanup no-op ===\n');
{ let ok = true; try { (require('../spells/hold_monster') as any).cleanup(makeCaster()); } catch { ok = false; } assert('no throw', ok); }
