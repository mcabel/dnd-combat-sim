// ============================================================
// Test: Friends Cantrip
// PHB p.244 — Level 0 enchantment cantrip (self-buff: advantage on next CHA check)
//
// v1 simplifications (all documented via metadata flags):
//   - Concentration: canonically concentration, up to 1 minute → v1: 1-round,
//     non-concentration (clears at start of caster's next turn).
//   - Target: canonically "directed at one creature of your choice that
//     isn't hostile toward you" → v1: target-agnostic (applies to next CHA
//     check regardless of target).
//   - Hostility backlash: canonically "the creature realizes that you used
//     magic to influence its mood and becomes hostile toward you" → v1:
//     backlash NOT implemented.
//   - CHA-check integration: the engine has NO rollAbilityCheck choke
//     point yet → v1 sets the flag on cast but does NOT consume it. The flag
//     is cleared at the start of the caster's NEXT turn via cleanup().
//
// Tests:
//   1. metadata correctness
//   2. metadata exposes components (S + M — makeup, NO V, canon per 5etools JSON)
//   3. metadata exposes isSelfBuff = true
//   4. metadata exposes riderCheckAbility = 'cha'
//   5. metadata exposes v1 simplification flags (concentration + target-agnostic + hostility-backlash + CHA-check-integration)
//   6. metadata does NOT scale (advantage buff is flat)
//   7. applySelfEffect sets _friendsAdvNextChaCheck = true + emits log
//   8. resolveCantripAction integration — 'Friends' routes to applySelfEffect
//   9. dispatcher safety — unknown cantrip name is a no-op
//  10. cleanup() clears _friendsAdvNextChaCheck flag (resetBudget integration)
//  11. v1 does NOT consume the flag (no rollAbilityCheck choke point yet)
//  12. buff clears at start of caster's next turn (resetBudget)
//  13. Friends itself does NOT go through resolveAttack (self-buff)
//  14. resolveCantripAoE returns false (not a caster-centered AoE)
//  15. resolveCantripTouchEffect returns false (not a touch-effect)
//  16. no CANTRIP_EFFECTS entry (not a post-hit rider)
//  17. mirror True Strike architecture (same one-shot semantics, but for CHA checks instead of attack rolls)
//  18. flavor log emitted on cast (mentions Friends + Charisma check)
//
// Run: npx ts-node src/test/friends.test.ts
// ============================================================

import { metadata, applySelfEffect, cleanup } from '../spells/friends';
import {
  applyCantripEffect as dispatchCantrip,
  resolveCantripAction,
  resolveCantripAoE,
  resolveCantripTouchEffect,
} from '../engine/cantrip_effects';
import { resetBudget } from '../engine/utils';
import { CombatEvent } from '../engine/combat';
import { Combatant, Action, Vec3, Cell, Obstacle } from '../types/core';

let passed = 0, failed = 0;

function assert(label: string, cond: boolean, detail: any = ''): void {
  if (cond) { console.log(`  ✅ ${label}`); passed++; }
  else       { console.error(`  ❌ ${label}${detail ? ' — ' + detail : ''}`); failed++; }
}
function eq<T>(label: string, a: T, b: T): void {
  assert(label, a === b, `got ${JSON.stringify(a)}, want ${JSON.stringify(b)}`);
}

function makeCombatant(id: string, overrides: Partial<Combatant> = {}): Combatant {
  return {
    id, name: id, isPlayer: false, faction: 'party',
    maxHP: 40, currentHP: 40, ac: 14, speed: 30,
    flySpeed: null, swimSpeed: null, burrowSpeed: null,
    str: 10, dex: 10, con: 10, int: 10, wis: 14, cha: 10,
    cr: 1,
    pos: { x: 0, y: 0, z: 0 },
    actions: [], traits: [], legendaryActions: [], legendaryActionPool: 0,
    legendaryActionPoolMax: 0,
    budget: { movementFt: 30, actionUsed: false, bonusActionUsed: false, reactionUsed: false, freeObjectUsed: false },
    conditions: new Set(),
    aiProfile: 'smart',
    perception: { targets: new Map() } as any,
    concentration: null,
    deathSaves: null,
    resources: null,
    tempHP: 0,
    mountedOn: null, carriedBy: null, independentMount: false,
    role: 'regular', bonded: null,
    usedSneakAttackThisTurn: false, helpedThisTurn: false,
    isDefender: false, cannotAttack: false, hasHands: true, wearingArmor: false,
    isDead: false, isUnconscious: false,
    advantages: [], vulnerabilities: [], resistances: [],
    bardicInspirationDie: null,
    wardingBond: null,
    activeEffects: [],
    ...overrides,
  };
}

function makeBF(combatants: Combatant[], obstacles: Obstacle[] = []) {
  const width = 10, height = 10, depth = 1;
  const cells: Cell[][][] = [];
  for (let x = 0; x < width; x++) {
    cells[x] = [];
    for (let y = 0; y < height; y++) {
      cells[x][y] = [];
      for (let z = 0; z < depth; z++) {
        cells[x][y][z] = { terrain: 'normal', elevation: 0 };
      }
    }
  }
  return {
    width, height, depth, cells,
    round: 1,
    combatants: new Map(combatants.map(c => [c.id, c])),
    initiativeOrder: combatants.map(c => c.id),
    obstacles: obstacles.length ? obstacles : undefined,
  };
}

function makeState(bf: any): any {
  return {
    battlefield: bf,
    log: { events: [], winner: null, rounds: 0 },
    disengagedThisTurn: new Set(),
    damageThisRound: new Map(),
    noDamageRounds: new Map(),
    rageDamagedSinceLastTurn: new Set(),
  };
}

// A Friends Action — self-buff, no target, no attack roll.
const FRIENDS_ACTION: Action = {
  name: 'Friends',
  isMultiattack: false,
  attackType: 'special', // self-buff — not 'melee'/'ranged'/'spell'/'save'
  reach: 0,
  range: { normal: 0, long: 0 }, // Self
  hitBonus: null,
  damage: null, // no damage — the cantrip grants advantage on next CHA check
  damageType: null,
  saveDC: null,
  saveAbility: null,
  isAoE: false,
  isControl: false,
  requiresConcentration: false,
  slotLevel: 0,
  costType: 'action',
  legendaryCost: 0,
  description: 'Friends',
};

// ============================================================
// 1. metadata
// ============================================================
console.log('\n--- 1. metadata ---');
{
  eq('1a. name', metadata.name, 'Friends');
  eq('1b. level (cantrip)', metadata.level, 0);
  eq('1c. school', metadata.school, 'enchantment');
  eq('1d. rangeFt (0 — Self)', metadata.rangeFt, 0);
  eq('1e. damageDice null (no damage — buff only)', metadata.damageDice, null);
  eq('1f. damageType null', metadata.damageType, null);
  eq('1g. not concentration (v1 simplification)', metadata.concentration, false);
  eq('1h. castingTime', metadata.castingTime, 'action');
}

// ============================================================
// 2. components: S + M (makeup, NO V) — PHB p.244, canon per 5etools JSON
// ============================================================
console.log('\n--- 2. components ---');
{
  eq('2a. NO verbal component (canon 5etools JSON: {"s":true,"m":...})',
    metadata.components.v, false);
  eq('2b. somatic component', metadata.components.s, true);
  eq('2c. material component (makeup)', metadata.components.m, true);
}

// ============================================================
// 3. metadata exposes isSelfBuff = true
// ============================================================
console.log('\n--- 3. isSelfBuff ---');
{
  eq('3a. isSelfBuff = true', metadata.isSelfBuff, true);
}

// ============================================================
// 4. metadata exposes riderCheckAbility = 'cha'
// ============================================================
console.log('\n--- 4. riderCheckAbility ---');
{
  eq('4a. riderCheckAbility = "cha" (Charisma checks)', metadata.riderCheckAbility, 'cha');
}

// ============================================================
// 5. metadata exposes v1 simplification flags
// ============================================================
console.log('\n--- 5. v1 simplification flags ---');
{
  eq('5a. friendsConcentrationV1Simplified = true (canon: conc 1 min, v1: 1 round)',
    metadata.friendsConcentrationV1Simplified, true);
  eq('5b. friendsTargetAgnosticV1Simplified = true (canon: directed at one creature, v1: target-agnostic)',
    metadata.friendsTargetAgnosticV1Simplified, true);
  eq('5c. friendsHostilityBacklashV1Implemented = false (canon: backlash on end; v1: skipped)',
    metadata.friendsHostilityBacklashV1Implemented, false);
  eq('5d. friendsAbilityCheckIntegrationV1Implemented = false (no rollAbilityCheck choke point yet)',
    metadata.friendsAbilityCheckIntegrationV1Implemented, false);
}

// ============================================================
// 6. metadata does NOT scale (advantage buff is flat)
// ============================================================
console.log('\n--- 6. no scaling ---');
{
  eq('6a. scales = false (Friends does NOT scale at 5/11/17)',
    metadata.scales, false);
}

// ============================================================
// 7. applySelfEffect sets _friendsAdvNextChaCheck = true + emits log
// ============================================================
console.log('\n--- 7. applySelfEffect ---');
{
  const caster = makeCombatant('bard');
  const bf = makeBF([caster]);
  const state = makeState(bf);

  eq('7a. flag not set before cast', caster._friendsAdvNextChaCheck, undefined);

  const ret = applySelfEffect(caster, state);
  eq('7b. applySelfEffect returns true', ret, true);
  eq('7c. _friendsAdvNextChaCheck set to true',
    caster._friendsAdvNextChaCheck, true);

  const castLog = state.log.events.find(
    (e: CombatEvent) => e.type === 'action' && e.description.includes('Friends'),
  );
  assert('7d. cast log emitted', castLog !== undefined,
    `events: ${state.log.events.map((e: CombatEvent) => e.description).join(' | ')}`);
  assert('7e. log mentions "advantage"',
    castLog?.description.toLowerCase().includes('advantage') === true,
    `got: ${castLog?.description}`);
  assert('7f. log mentions "Charisma" or "CHA"',
    castLog?.description.toLowerCase().includes('charisma') === true ||
    castLog?.description.toLowerCase().includes('cha') === true,
    `got: ${castLog?.description}`);
}

// ============================================================
// 8. resolveCantripAction integration — 'Friends' routes to applySelfEffect
// ============================================================
console.log('\n--- 8. resolveCantripAction integration ---');
{
  const caster = makeCombatant('bard');
  const bf = makeBF([caster]);
  const state = makeState(bf);

  const ret = resolveCantripAction(caster, 'Friends', state);
  eq('8a. resolveCantripAction returns true', ret, true);
  eq('8b. _friendsAdvNextChaCheck set to true',
    caster._friendsAdvNextChaCheck, true);
}

// ============================================================
// 9. dispatcher safety — unknown cantrip name is a no-op
// ============================================================
console.log('\n--- 9. dispatcher safety ---');
{
  const caster = makeCombatant('bard');
  const target = makeCombatant('goblin');
  const bf = makeBF([caster, target]);
  const state = makeState(bf);

  dispatchCantrip(caster, target, 'Definitely Not A Cantrip', state);
  eq('9a. unknown cantrip → no log events', state.log.events.length, 0);
  eq('9b. unknown cantrip → no flag set on caster',
    caster._friendsAdvNextChaCheck, undefined);
  eq('9c. unknown cantrip → no flag set on target',
    target._friendsAdvNextChaCheck, undefined);
}

// ============================================================
// 10. cleanup() clears _friendsAdvNextChaCheck flag (resetBudget integration)
// ============================================================
console.log('\n--- 10. cleanup ---');
{
  const caster = makeCombatant('bard', { _friendsAdvNextChaCheck: true });
  cleanup(caster);
  eq('10a. _friendsAdvNextChaCheck cleared by cleanup()',
    caster._friendsAdvNextChaCheck, undefined);

  // resetBudget integration — buff clears at start of caster's next turn.
  const caster2 = makeCombatant('bard2', { _friendsAdvNextChaCheck: true });
  resetBudget(caster2);
  eq('10b. _friendsAdvNextChaCheck cleared by resetBudget()',
    caster2._friendsAdvNextChaCheck, undefined);
}

// ============================================================
// 11. v1 does NOT consume the flag (no rollAbilityCheck choke point yet)
// ============================================================
console.log('\n--- 11. v1 does NOT consume flag ---');
{
  // In v1, there is no rollAbilityCheck choke point in utils.ts. The flag
  // is set on cast and only cleared by cleanup() at the start of the
  // caster's NEXT turn. The flag is NEVER consumed by a CHA check
  // in v1 (because the choke point doesn't exist yet).
  const caster = makeCombatant('bard', { _friendsAdvNextChaCheck: true });
  eq('11a. flag set', caster._friendsAdvNextChaCheck, true);

  // Simulate "a CHA check happens" — in v1, nothing consumes the flag.
  // (In the future, rollAbilityCheck would consume it here.)
  eq('11b. flag STILL set in v1 (no rollAbilityCheck choke point to consume it)',
    caster._friendsAdvNextChaCheck, true);

  // Cleanup is the ONLY mechanism that clears the flag in v1.
  cleanup(caster);
  eq('11c. flag cleared by cleanup (v1: only clearing mechanism)',
    caster._friendsAdvNextChaCheck, undefined);
}

// ============================================================
// 12. buff clears at start of caster's next turn (resetBudget)
// ============================================================
console.log('\n--- 12. buff clears at start of next turn ---');
{
  const caster = makeCombatant('bard', { _friendsAdvNextChaCheck: true });
  eq('12a. flag set before resetBudget', caster._friendsAdvNextChaCheck, true);

  resetBudget(caster); // simulates start of caster's next turn

  eq('12b. flag cleared after resetBudget', caster._friendsAdvNextChaCheck, undefined);
}

// ============================================================
// 13. Friends itself does NOT go through resolveAttack (self-buff)
// ============================================================
console.log('\n--- 13. Friends bypasses resolveAttack ---');
{
  const caster = makeCombatant('bard', {
    pos: { x: 0, y: 0, z: 0 },
    actions: [FRIENDS_ACTION],
  });
  const bf = makeBF([caster]);
  const state = makeState(bf);

  resolveCantripAction(caster, 'Friends', state);

  // Only the "casts Friends" action event should be present.
  const actionEvents = state.log.events.filter((e: CombatEvent) => e.type === 'action');
  eq('13a. exactly 1 action event (the cast)', actionEvents.length, 1);
  assert('13b. action event mentions Friends',
    actionEvents[0]?.description.includes('Friends') === true,
    `got: ${actionEvents[0]?.description}`);

  // No attack hit/miss/crit/damage events.
  const attackEvents = state.log.events.filter((e: CombatEvent) =>
    e.type === 'attack_hit' || e.type === 'attack_miss' || e.type === 'attack_crit' || e.type === 'damage',
  );
  eq('13c. no attack/damage events (self-buff bypasses resolveAttack)',
    attackEvents.length, 0);
}

// ============================================================
// 14. resolveCantripAoE returns false (not a caster-centered AoE)
// ============================================================
console.log('\n--- 14. not a caster-centered AoE ---');
{
  const caster = makeCombatant('bard');
  const bf = makeBF([caster]);
  const state = makeState(bf);

  const ret = resolveCantripAoE(caster, 'Friends', state);
  eq('14a. resolveCantripAoE returns false', ret, false);
  eq('14b. no log events', state.log.events.length, 0);
}

// ============================================================
// 15. resolveCantripTouchEffect returns false (not a touch-effect)
// ============================================================
console.log('\n--- 15. not a touch-effect ---');
{
  const caster = makeCombatant('bard');
  const target = makeCombatant('goblin');
  const bf = makeBF([caster, target]);
  const state = makeState(bf);

  const ret = resolveCantripTouchEffect(caster, target, 'Friends', state);
  eq('15a. resolveCantripTouchEffect returns false', ret, false);
  eq('15b. no log events', state.log.events.length, 0);
}

// ============================================================
// 16. no CANTRIP_EFFECTS entry (not a post-hit rider)
// ============================================================
console.log('\n--- 16. no CANTRIP_EFFECTS entry ---');
{
  const caster = makeCombatant('bard');
  const target = makeCombatant('goblin');
  const bf = makeBF([caster, target]);
  const state = makeState(bf);

  const eventsBefore = state.log.events.length;
  dispatchCantrip(caster, target, 'Friends', state);
  eq('16a. dispatcher no-op (no log events added)',
    state.log.events.length, eventsBefore);
  eq('16b. no flag set on target', target._friendsAdvNextChaCheck, undefined);
}

// ============================================================
// 17. mirror True Strike architecture (same one-shot semantics, but for CHA checks instead of attack rolls)
// ============================================================
console.log('\n--- 17. mirror True Strike architecture ---');
{
  // Friends and True Strike share the same architecture:
  //   - Same one-shot consume semantics (consumed by the first check/attack)
  //   - Same cleanup() pattern (clears at start of caster's next turn)
  //   - Same self-buff routing (CANTRIP_SELF_EFFECTS)
  //   - Same flag value type (boolean = true)
  //
  // The ONLY differences:
  //   - Friends: CHA-check advantage (consumed by FUTURE rollAbilityCheck)
  //   - True Strike: attack-roll advantage (consumed by EXISTING resolveAttack)
  //
  // Verify the architecture mirrors:
  const friendsCaster = makeCombatant('bard1');
  applySelfEffect(friendsCaster, makeState(makeBF([friendsCaster])));
  eq('17a. Friends flag value = true (mirror True Starc boolean flag)',
    friendsCaster._friendsAdvNextChaCheck, true);

  // Import True Strike for comparison.
  const { metadata: trueStrikeMeta, applySelfEffect: applyTrueStrike } =
    require('../spells/true_strike') as typeof import('../spells/true_strike');
  const trueStrikeCaster = makeCombatant('bard2');
  applyTrueStrike(trueStrikeCaster, makeState(makeBF([trueStrikeCaster])));
  eq('17b. True Strike flag value = true (mirror Friends boolean flag)',
    trueStrikeCaster._trueStrikeAdvNextAttack, true);

  eq('17c. same isSelfBuff (true)',
    metadata.isSelfBuff, trueStrikeMeta.isSelfBuff);
  eq('17d. same scales (false — neither scales at 5/11/17)',
    metadata.scales, trueStrikeMeta.scales);
  eq('17e. same concentration (false — both v1: 1-round non-concentration)',
    metadata.concentration, trueStrikeMeta.concentration);
}

// ============================================================
// 18. flavor log emitted on cast (mentions Friends + Charisma check)
// ============================================================
console.log('\n--- 18. flavor log ---');
{
  const caster = makeCombatant('bard', { name: 'Bard Alice' });
  const bf = makeBF([caster]);
  const state = makeState(bf);

  applySelfEffect(caster, state);

  const castLog = state.log.events.find(
    (e: CombatEvent) => e.type === 'action' && e.description.includes('Friends'),
  );
  assert('18a. cast log mentions caster name',
    castLog?.description.includes('Bard Alice') === true,
    `got: ${castLog?.description}`);
  assert('18b. cast log mentions "casts Friends"',
    castLog?.description.includes('casts Friends') === true,
    `got: ${castLog?.description}`);
  assert('18c. cast log mentions "Charisma" or "CHA"',
    castLog?.description.toLowerCase().includes('charisma') === true ||
    castLog?.description.toLowerCase().includes('cha') === true,
    `got: ${castLog?.description}`);
}

// ============================================================
// Summary
// ============================================================
console.log(`\nResults: ${passed} passed, ${failed} failed`);
if (failed > 0) process.exit(1);
