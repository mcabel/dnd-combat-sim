...
//
// Run: npx ts-node --transpile-only src/test/banishing_smite.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/banishing_smite';
...
eq('1a: name', metadata.name, 'Banishing Smite');
eq('1b: level', metadata.level, 5);
eq('1c: concentration true', metadata.concentration, true);
eq('1d: dieSides 10', metadata.dieSides, 10);
eq('1e: count 5', metadata.count, 5);
eq('1f: damageType force', metadata.damageType, 'force');
eq('1g: canon flag true', metadata.banishingSmiteCanonV1Implemented, true);
eq('1h: riders-simplified flag true', metadata.banishingSmiteRidersV1Simplified, true);
...

{
  ...
  const slotsBefore = caster.resources!.spellSlots![5].remaining;
  execute(caster, state);
  const slotsAfter = caster.resources!.spellSlots![5].remaining;
...

{
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Banishing Smite');
}

{
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Banishing Smite');
  eq('3e: rider.dieSides', caster._nextHitRider!.dieSides, 10);
  eq('3f: rider.count', caster._nextHitRider!.count, 5);
  eq('3g: rider.damageType', caster._nextHitRider!.damageType, 'force');
  assert('3h: no condition (banish-if-HP-≤-50 rider simplified)',
    caster._nextHitRider!.condition === undefined);
}

{
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Banishing Smite'));
...

{
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Banishing Smite'));
...

{
  ...
  caster._nextHitRider = {
    ...
    dieSides: 10, count: 5, damageType: 'force',
  };
...

{
  ...
  caster.concentration = { active: true, spellName: 'Banishing Smite', dcIfHit: 10 };
  cleanup(caster);
