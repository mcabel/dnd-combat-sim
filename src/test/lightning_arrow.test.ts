...
//
// Run: npx ts-node --transpile-only src/test/lightning_arrow.test.ts
// ============================================================

import { shouldCast, execute, cleanup, metadata } from '../spells/lightning_arrow';
...
eq('1a: name', metadata.name, 'Lightning Arrow');
...
eq('1g: canon flag true', metadata.lightningArrowCanonV1Implemented, true);
eq('1h: riders-simplified flag true', metadata.lightningArrowRidersV1Simplified, true);
...

{
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Lightning Arrow');
}

{
  ...
  eq('3d: rider.spellName', caster._nextHitRider!.spellName, 'Lightning Arrow');
  eq('3e: rider.dieSides', caster._nextHitRider!.dieSides, 8);
  eq('3f: rider.count', caster._nextHitRider!.count, 4);
  eq('3g: rider.damageType', caster._nextHitRider!.damageType, 'lightning');
  assert('3h: no condition (AoE splash simplified)',
...

{
  ...
  const actionEvent = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Lightning Arrow'));
...

{
  ...
  const condEvent = state.log.events.find(
    e => e.type === 'condition_add' && e.description.includes('Lightning Arrow'));
...

{
  ...
  caster._nextHitRider = {
    ...
    dieSides: 8, count: 4, damageType: 'lightning',
  };
...

{
  ...
  caster.concentration = { active: true, spellName: 'Lightning Arrow', dcIfHit: 10 };
  cleanup(caster);
