...
const CD_ACTION: Action = { name: 'Compelled Duel', isMultiattack: false, attackType: 'save', reach: 5, range: { normal: 30, long: 30 }, hitBonus: null, damage: null, damageType: null, saveDC: 25, saveAbility: 'wis', isAoE: false, isControl: true, requiresConcentration: true, slotLevel: 1, costType: 'action', legendaryCost: 0, description: 'Compelled Duel' };
const CD_ACTION_LOW: Action = { ...CD_ACTION, saveDC: 5 };
...
function makeCaster(pos: any = { x: 0, y: 0, z: 0 }, a: Action = CD_ACTION) { return makeCombatant('pal', { name: 'Caster', pos, actions: [a], resources: withSlots1(1) }); }
...
console.log('\n=== 1. Metadata ===\n');
eq('Name', metadata.name, 'Compelled Duel'); eq('Level 1', metadata.level, 1); eq('Range 30', metadata.rangeFt, 30); eq('Save wis', metadata.saveAbility, 'wis'); eq('Concentration', metadata.concentration, true);
console.log('\n=== 2. shouldCast gates ===\n');
{ const c = makeCombatant('pal', { actions: [], resources: withSlots1(1) }); eq('null: no action', shouldCast(c, makeBF([c, weak('e1', { x: 1, y: 0 })])), null); }
{ const c = makeCombatant('pal', { actions: [CD_ACTION], resources: withSlots1(0) }); eq('null: no slots', shouldCast(c, makeBF([c, weak('e1', { x: 1, y: 0 })])), null); }
