// ============================================================
// intellect_fortress.test.ts — Intellect Fortress spell module
// TCE p.107: 3rd-level abjuration, 1 action, range Self (30-ft aura),
// concentration (1 hr).
// Effect: target gains advantage on INT/WIS/CHA saves + psychic resistance
// (v1 simplified: scope 'save' = all saves; psychic resistance NOT modelled).
//
...
import { shouldCast, execute, metadata, cleanup } from '../spells/intellect_fortress';
...
const IF_ACTION: Action = {
  ...
  costType: 'action',
  legendaryCost: 0,
  description: 'Intellect Fortress',
};
...
eq('1: name is Intellect Fortress', metadata.name, 'Intellect Fortress');
...
eq('1: canon flag set', (metadata as any).intellectFortressCanonV1Implemented, true);
eq('1: scope-simplified flag set', (metadata as any).intellectFortressScopeV1SimplifiedToAllSaves, true);
eq('1: psychic-resistance not-modelled flag set', (metadata as any).intellectFortressPsychicResistanceV1NotModelled, true);
...
{
  // 2d: returns null when no Intellect Fortress action
  const caster = makeCaster('wizard');
  caster.actions = [];
  const ally = makeAlly('fighter');
  const bf   = makeBF([caster, ally]);
  assert('2d: null when no Intellect Fortress action', shouldCast(caster, bf) === null);
}
...
{
  // 3b: concentration started on Intellect Fortress
  const caster = makeCaster('wizard');
  const ally   = makeAlly('fighter');
  const bf     = makeBF([caster, ally]);
  const state  = makeState(bf);
  ...
  eq('3b: concentration.spellName', caster.concentration?.spellName, 'Intellect Fortress');
}

{
  // 3c: advantage on ALL saves applied to each target via grantSelf(scope='save')
  // (v1 simplification: canon is INT/WIS/CHA only — flag set in metadata)
  const caster = makeCaster('wizard');
  ...
  // querySelf returns advantage=true for ALL save scopes (general 'save' covers
  // 'save:int', 'save:wis', 'save:cha', 'save:str', 'save:dex', 'save:con')
  const scopes: Array<'save' | 'save:int' | 'save:wis' | 'save:cha' | 'save:dex'> =
    ['save', 'save:int', 'save:wis', 'save:cha', 'save:dex'];
  let allAdv = true;
  for (const sc of scopes) {
    if (!querySelf(ally1, sc).advantage) { allAdv = false; break; }
  }
  assert('3c: ally1 has advantage on ALL save scopes (simplified)', allAdv);
...
{
  // 3d: advantage does NOT bleed into attack rolls (scope is 'save' only)
  const caster = makeCaster('wizard');
  const ally   = makeAlly('fighter');
  const bf     = makeBF([caster, ally]);
  const state  = makeState(bf);
...

{
  ...
  const effect = ally.activeEffects.find(e => e.spellName === 'Intellect Fortress');
  assert('3e: ally has Intellect Fortress activeEffect', effect !== undefined);
...

{
  ...
  const actionEvt = state.log.events.find(
    e => e.type === 'action' && e.description.includes('Intellect Fortress'));
...

{
  ...
  assert('3g: ally has activeEffect before break',
    ally.activeEffects.some(e => e.spellName === 'Intellect Fortress'));
...
{
  const caster = makeCaster('wizard');
  caster.concentration = { active: true, spellName: 'Intellect Fortress', dcIfHit: 10 };
  cleanup(caster);
  eq('4: cleanup does NOT break concentration', caster.concentration?.active, true);
  eq('4: cleanup does NOT change spellName', caster.concentration?.spellName, 'Intellect Fortress');
}
