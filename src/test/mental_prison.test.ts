// ============================================================
// mental_prison.test.ts — Mental Prison bespoke spell module (Session 24)
// XGE p.161: 6th-level illusion, action, range 60 ft. Canon: concentration,
// up to 1 minute; movement-trigger 3d10. v1: concentration +
// movement-trigger simplified to one-shot.
// Effect: single-target INT save. On fail: 5d10 psychic. On success: half.
//
...
// Mental Prison's stats (L6, INT save, 5d10 psychic, 60-ft range, v1 one-shot).
// Uses withSlots6.
//
...
import { shouldCast, execute, metadata, rollDamage } from '../spells/mental_prison';
...
const MP_ACTION: Action = {
  ...
  costType: 'action',
  legendaryCost: 0,
  description: 'Mental Prison (INT save, 5d10 psychic, 60-ft range, single-target)',
};

const MP_ACTION_LOW_DC: Action = {
  ...MP_ACTION,
...
/** Wizard at pos (0,0,0) with Mental Prison + 2 6th-level slots */
function makeCaster(pos: Vec3 = { x: 0, y: 0, z: 0 }, action: Action = MP_ACTION): Combatant {
...
eq('Name is Mental Prison', metadata.name, 'Mental Prison');
...
// 2a. No Mental Prison action → null
{
  const caster = makeCombatant('wiz', { actions: [], resources: withSlots6(2) });
  const enemy = makeWeakEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
  eq('Returns null when caster lacks Mental Prison action', shouldCast(caster, bf), null);
}
// 2b. No 6th-level slots → null
{
  const caster = makeCombatant('wiz', { actions: [MP_ACTION], resources: withSlots6(0) });
  const enemy = makeWeakEnemy('e1', { x: 1, y: 0, z: 0 });
  const bf = makeBF([caster, enemy]);
...

{
  ...
  if (target) {
    ...
    // No condition rider for Mental Prison (v1 one-shot, no restrained in v1)
    const condAdds = state.log.events.filter((e: any) => e.type === 'condition_add');
    eq('No condition-add logs (v1 one-shot simplification)', condAdds.length, 0);
  }
}
...
{
  const caster = makeCaster({ x: 0, y: 0, z: 0 }, MP_ACTION_LOW_DC);
  ...
  if (target) {
    ...
    // 5a. Damage applied (half of 5d10, floor → range 2-25)
    const dmgDealt = hpBefore - enemy.currentHP;
...

{
  ...
  if (target) {
    ...
    // Only ONE enemy takes damage — Mental Prison is single-target
...

{
  ...
  let cleanupOk = true;
  try { (require('../spells/mental_prison') as any).cleanup(caster); }
